...
const json = require('@/com/json/story2.json');
...
const botImg = require('./botavator.png');
...
export default function DialogGame(props: DialogGameProps) {
  ...
  const [inkStory, setInky] = useState<any>(null);
  useEffect(() => {
    setInky(new Story(json));
  }, []);
  useEffect(() => {
    if (inkStory) {
      storyGo();
  ...
  function storyGo() {
    ...
    if (!currentChoices || (currentChoices.length === 0 && currentContext === '')) {
      props.closeCb();
      return;
    }
  ...
  return(
    <div className="dialog-container">
      ...
      <div className="bot-dialog">
        <img className="bot-avator" src={botImg}/>
